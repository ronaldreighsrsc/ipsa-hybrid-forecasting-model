...
class BiLSTMTrainer:
    """
    Motor de Deep Learning usando Bidirectional Long Short-Term Memory (BiLSTM).
    Procesa secuencias temporales en ambas direcciones (pasado->futuro y futuro->pasado).
    Maneja internamente la conversión a tensores 3D, escalamiento local,
    Purged CV y Walk-Forward con limpieza estricta de memoria RAM.
    """
    ...
    def prepare_sequences(self, df: pd.DataFrame, target_col: str, feature_cols: list) -> tuple:
        ...
        for i in range(self.look_back, len(dataset)):
            # Mantenemos el formato 2D por ventana para formar el Tensor 3D final
    ...
    def _build_model(self, input_shape: tuple, units: int, dropout: float) -> Sequential:
        """Arquitectura base de la red neuronal Bidireccional."""
        model = Sequential()
        model.add(Input(shape=input_shape))
        
        # Capa Bidireccional Principal
        model.add(Bidirectional(LSTM(units, return_sequences=True)))
        model.add(Dropout(dropout))
        
        # Capa Bidireccional Secundaria
        ...
        # Ojo: Mantenemos tu learning_rate de 0.0005 específico para el BiLSTM
        model.compile(optimizer=Adam(learning_rate=0.0005), loss='binary_crossentropy', metrics=['accuracy'])
        return model

    def find_best_params(self, X_train: np.ndarray, y_train: np.ndarray, param_grid: list) -> dict:
        """Grid Search 3D con K-Fold Purged & Embargoed."""
        ...
        for params in param_grid:
            ...
            for train_idx, val_idx in folds:
                ...
                model_cv = self._build_model((t_steps, n_feat), units=params['units'], dropout=params['dropout'])
        ...
        for i in range(len(X_test_scaled)):
            ...
            if (i + 1) % self.retrain_step == 0:
                if (i+1) % (self.retrain_step*2) == 0:
                    print(f"    > Re-calibrando pesos BiLSTM en paso {i+1}/{len(X_test_scaled)}...")
